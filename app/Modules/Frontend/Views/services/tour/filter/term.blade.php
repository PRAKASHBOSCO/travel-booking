                ...
                <div class="row gmz-checkbox-wrapper">
                    @foreach($terms as $key => $term)
                        <div class="col-md-4 col-6">
                            <label class="checkbox-inline"><input type="checkbox" class="gmz-checkbox-item" name="tour_{{$tax}}s[]" value="{{$key}}" @if(in_array($key, $post_data)) checked @endif>{{get_translate($term)}}</label>
                        </div>
                    @endforeach
