...
class CountriesService extends AbstractService implements ICountriesService{

    protected $repository;
    private $seoRepo;

    public function __construct(ICountriesRepository $repository, ISeoRepository $seoRepo)
    {
        $this->repository = $repository;
        $this->seoRepo = $seoRepo;
    }

     public function getCountriesPagination($number = 10, $where = []){
    ...
    public function deleteCountryTemp()
    {
    ...
    public function storeNewCountry()
    {
        $dt=date('Y-m-d H:i:s');
    ...
    public function deleteCountries($request){
        ...
        // dd($post_data);
           ...
           $updated = $this->repository->update($post_id, $post_data);
            if($updated){
                ...
                if($finish) {
                    $response['redirect'] = dashboard_url('all-countries' /*. $post_id */);
                }
