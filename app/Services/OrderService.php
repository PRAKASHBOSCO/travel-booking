...
class OrderService extends AbstractService implements IOrderService
{
   protected $repository;
   private $carRepo;
   private $carAvaiRepo;
   private $apartmentRepo;
   private $apartmentAvaiRepo;
   private $tourRepo;
   private $tourAvaiRepo;
   private $spaceRepo;
   private $spaceAvaiRepo;
   private $userRepo;
   private $roleUserRepo;
   private $hotelRepo;
   private $roomRepo;
   private $roomAvaiRepo;
   private $beautyRepo;
   private $beautyAvaiRepo;
   private $beautyService;
   private $agentAvaiRepo;

   public function __construct(IOrderRepository $repository, ICarRepository $carRepo, ICarAvailabilityRepository $carAvaiRepo, IApartmentRepository $apartmentRepo, IApartmentAvailabilityRepository $apartmentAvaiRepo, ITourRepository $tourRepo, ITourAvailabilityRepository $tourAvaiRepo, ISpaceRepository $spaceRepo, ISpaceAvailabilityRepository $spaceAvaiRepo, IUserRepository $userRepo, IRoleUserRepository $roleUserRepo, IHotelRepository $hotelRepo, IRoomAvailabilityRepository $roomAvaiRepo, IRoomRepository $roomRepo, IBeautyRepository $beautyRepo, IBeautyAvailabilityRepository $beautyAvaiRepo, IBeautyService $beautyService, IAgentAvailabilityRepository $agentAvaiRepo)
   {
   ...
   public function checkingBeforeCheckout($serviceObject, $cart)
   {
      if ($cart['post_type'] == GMZ_SERVICE_CAR) {
         ...
         $slot_data = $this->beautyService->getSlotEmptyByDay($cart['post_id'], $cart['cart_data']['check_in']);
         $check_out = false;
   ...
   public function checkOut(Request $request)
   {
      ...
      if (!empty($cart)) {
         ...
         $serviceRepo = $post_type . 'Repo';
         $serviceObject = $this->$serviceRepo->find($cart['post_id']);
         if ($serviceObject) {
            ...
            if (in_array($post_type, [GMZ_SERVICE_APARTMENT, GMZ_SERVICE_TOUR, GMZ_SERVICE_SPACE, GMZ_SERVICE_CAR, GMZ_SERVICE_BEAUTY])) {
               $serviceAvaiRepo = $post_type . 'AvaiRepo';
               $check_avail = $this->$serviceAvaiRepo->checkAvailability($cart['post_id'], $cart_data['check_in'], $cart_data['check_out']);
               if (!$check_avail) {
                  return [
                     'status' => false,
               ...
               if (!empty($rooms)) {
                  foreach ($rooms as $k => $v) {
                     if ($v['number'] > 0) {
                        $avail = $this->roomAvaiRepo->checkAvailabilityWithGuest($k, $cart_data['check_in'], $cart_data['check_out'], $v['number'], $cart_data['adult'], $cart_data['children']);
                        if ($avail->isEmpty()) {
                           $room_avails[] = $k;
                        }
                     }
            ...
            if (!is_user_login()) {
               $user_exists = $this->userRepo->where(['email' => $post_data['email']], true);
               ...
               $user_id = $this->userRepo->create($user_data);

               if ($user_id) {
                  $this->roleUserRepo->create([
                     'role_id' => 3,
                     'user_id' => $user_id,
                  ]);
            ...
            if (isset($after_payment['status']) && $after_payment['status']) {
               if ($post_type == GMZ_SERVICE_HOTEL) {
                  $this->roomAvaiRepo->updateBookedData($cart_data['check_in'], $cart_data['check_out'], $cart_data['rooms']);
               }elseif ($post_type == GMZ_SERVICE_CAR) {
                    $this->carAvaiRepo->updateBookedData($cart_data['check_in'], $cart_data['check_out'], $cart_data['number'], $serviceObject);
               }elseif ($post_type == GMZ_SERVICE_APARTMENT) {
                  $this->apartmentAvaiRepo->updateBookedData($cart_data['check_in'], $cart_data['check_out'], $serviceObject);
               }elseif ($post_type == GMZ_SERVICE_SPACE) {
                   $this->spaceAvaiRepo->updateBookedData($cart_data['check_in'], $cart_data['check_out'], $serviceObject);
               } elseif ($post_type == GMZ_SERVICE_TOUR) {
                  $this->tourAvaiRepo->updateBookedData($cart_data['check_in'], $cart_data['check_out'], $serviceObject, $cart['adult'], $cart['children']);
               } elseif ($post_type == GMZ_SERVICE_BEAUTY){
                  $this->agentAvaiRepo->updateBookedData($cart_data['check_in'], $cart_data['check_out'], $cart_data['agent_id'], $order_id, 'booked');
               }
            }
      ...
      if (!empty($order) && ($status_order !== $order['status'])) {
         ...
         if($order['post_type'] == GMZ_SERVICE_BEAUTY && !empty($response)){
            if ($status_order == GMZ_STATUS_CANCELLED || $status_order == GMZ_STATUS_REFUNDED){
               //unbooked
               $this->agentAvaiRepo->deleteByWhere(['order_id' => $order_id]);
               ...
               $this->agentAvaiRepo->updateBookedData($order['start_date'], $order['end_date'], $cart_data['agent_id'],$order_id,'booked');
            }
         }
   ...
   public function checkPaymentCancellation($order)
   {
      ...
      if ($post_type == GMZ_SERVICE_CAR) {
         $p = $this->carRepo->find($post_id);
      } elseif ($post_type == GMZ_SERVICE_APARTMENT) {
         $p = $this->apartmentRepo->find($post_id);
      } elseif ($post_type == GMZ_SERVICE_TOUR) {
         $p = $this->tourRepo->find($post_id);
      } elseif ($post_type == GMZ_SERVICE_SPACE) {
         $p = $this->spaceRepo->find($post_id);
      } elseif ($post_type == GMZ_SERVICE_HOTEL) {
         $p = $this->hotelRepo->find($post_id);
      }
