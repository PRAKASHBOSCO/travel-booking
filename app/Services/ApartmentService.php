...
class ApartmentService extends AbstractService implements IApartmentService
{
   protected $repository;
   protected $apartmentAvaiRepo;
   private $termRelationRepo;
   private $termRepo;
   private $orderRepo;
   private $commentRepo;
    private $seoRepo;

   public function __construct(IApartmentRepository $repository, IApartmentAvailabilityRepository $apartmentAvaiRepo, ITermRelationRepository $termRelationRepo, ITermRepository $termRepo, IOrderRepository $orderRepo, ICommentRepository $commentRepo, ISeoRepository $seoRepo)
   {
   ...
   public function fetchTime(Request $request)
   {
      ...
      if ($start_date && gmz_compare_hashing($postID, $postHashing)) {
         $orderData = $this->orderRepo->getOrderItems($postID, $start_date, $start_date, GMZ_SERVICE_APARTMENT);
   ...
   public function sendEnquiry(Request $request)
   {
      ...
      if (!empty($post_object)) {
         ...
         //\GMZ_Mail::inst()->sendEmailApartmentEnquiry($post_object, $request->all());
   ...
   public function getRealPrice($post_id, $check_in, $check_out, $extras)
   {
      ...
      if ($booking_type == 'per_day') {
         ...
         $avails = $this->apartmentAvaiRepo->getDataAvailability($post_id, $check_in, $check_out);
         ...
         $orders = $this->orderRepo->getOrderItemsWithTime($post_id, $check_in, $check_out, $startTime, $endTime, 'apartment', $booking_type);
   ...
   public function fetchApartmentAvailability(Request $request)
   {
      ...
      if ($start_date && $end_date && gmz_compare_hashing($postID, $postHashing)) {
         $avails = $this->apartmentAvaiRepo->getDataAvailability($postID, $start_date, $end_date);
         $apartment = $this->repository->find($postID);
         $price = (float)$apartment->base_price;
         if ($bookingType == 'per_day') {
            $orders = $this->orderRepo->getOrderItems($postID, $start_date, $end_date, GMZ_SERVICE_APARTMENT);
         } else {
            $orders = $this->orderRepo->getOrderItemsGroupDate($postID, $start_date, $end_date, GMZ_SERVICE_APARTMENT);
         }
   ...
   public function getSearchResult(Request $request)
   {
      ...
      if (!empty($params['checkIn']) && !empty($params['checkOut'])) {
         $unavail = $this->apartmentAvaiRepo->getListUnavailable($params);
   ...
   private function updateTerm($post_id, $post_data)
   {
      if (isset($post_data['apartment_type'])) {
         ...
         if (!empty($all_types)) {
            ...
            $this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'apartment' AND term_id IN {$type_in_str}");
         }
      ...
      if (isset($post_data['apartment_amenity'])) {
         ...
         if (!empty($all_amenities)) {
            ...
            $this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'apartment' AND term_id IN {$amenity_in_str}");
         }
   ...
   public function savePost($request)
   {
      ...
      if (!empty($post_id)) {
         ...
         if(isset($post_data['base_price'])){
             $this->apartmentAvaiRepo->updateByWhere([
                 'post_id' => $post_id,
