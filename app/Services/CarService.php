...
class CarService extends AbstractService implements ICarService {
	...
	public function __construct( ICarRepository $repository, ICarAvailabilityRepository $carAvaiRepo, ITermRelationRepository $termRelationRepo, ITermRepository $termRepo, IOrderRepository  $orderRepo, ICommentRepository $commentRepo, ISeoRepository $seoRepo) {
    ...
    public function getRealPrice($post_id, $check_in, $check_out, $number, $equipments, $insurances ) {
        ...
        if ($number > 0) {
            ...
            $avails      = $this->carAvaiRepo->getDataAvailability( $post_id, $check_in, $check_out );
            ...
            $orders = $this->orderRepo->getOrderItems($post_id, $check_in, $check_out, 'car');
    ...
    public function fetchCarAvailability(Request $request) {
        ...
        if ($start_date && $end_date && gmz_compare_hashing($postID, $postHashing)) {
            $avails = $this->carAvaiRepo->getDataAvailability($postID, $start_date, $end_date);
            $car = $this->repository->find($postID);
            $price = (float)$car->base_price;
            $quantity = (int)$car->quantity;
            $orders = $this->orderRepo->getOrderItems($postID, $start_date, $end_date, 'car');
	...
	private function updateTerm( $post_id, $post_data ) {
		if(isset($post_data['car_type'])){
			...
			if(!empty($all_types)){
				...
				$this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'car' AND term_id IN {$type_in_str}");
			}
		...
		if(isset($post_data['car_feature'])){
			...
			if(!empty($all_features)){
				...
				$this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'car' AND term_id IN {$feature_in_str}");
			}
        ...
        if(isset($post_data['equipments'])){
            ...
            if(!empty($all_equips)){
                ...
                $this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'car' AND term_id IN {$equip_in_str}");
            }
