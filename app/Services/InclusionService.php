...
class InclusionService extends AbstractService implements IInclusionService{

	protected $repository;
    private $seoRepo;

    public function __construct(IInclusionRepository $repository, ISeoRepository $seoRepo)
    {
        $this->repository = $repository;
        $this->seoRepo = $seoRepo;
    }

	 public function getInclusionPagination($number = 10, $where = []){
    ...
    public function deleteInclusionTemp()
    {
    ...
    public function storeNewInclusion()
    {
        $dt=date('Y-m-d H:i:s');

         return $this->repository->save([
            'title' => 'New Inclusion' . time(),
            'icon_img_upload' => '',
            'created_by' => get_current_user_id(),
    ...
    public function deleteInclusion($request){
        ...
        if(!empty($post_id)){
           ...
           $updated = $this->repository->update($post_id, $post_data);
            ...
            if($updated){
                ...
                // dd($finish);
                if($finish) {
                    $response['redirect'] = dashboard_url('all-inclusion' /*. $post_id */);
                }
