...
class PostService extends AbstractService implements IPostService
{
   ...
   public function __construct(IPostRepository $repository, ITermRelationRepository $termRelationRepo, ITermRepository $termRepo, ICommentRepository $commentRepo, ISeoRepository $seoRepo)
   {
   ...
   private function updateTerm($post_id, $post_data)
   {
      if (isset($post_data['post_category'])) {
         ...
         if (!empty($all_categories)) {
            ...
            $this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'post' AND term_id IN {$cate_in_str}");
         }
      ...
      if (isset($post_data['post_tag'])) {
         ...
         if (!empty($all_tags)) {
            ...
            $this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'post' AND term_id IN {$tag_in_str}");
         }
         ...
         if (!empty($post_tags)) {
            ...
            foreach ($post_tags as $tag) {
               $tag = trim($tag);
               if (!empty($all_tags)) {
                  $check_exists = $this->termRepo->whereRaw("term_title = '{$tag}' AND id IN {$tag_in_str}", true);
                  if ($check_exists) {
                     array_push($terms_id, $check_exists->id);
                  } else {
                     $new_tag = $this->termRepo->create([
                        'term_title' => $tag,
