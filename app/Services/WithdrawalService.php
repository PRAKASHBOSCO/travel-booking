...
class WithdrawalService extends AbstractService implements IWithdrawalService
{
   protected $repository;
   protected $earningsRepository;
   protected $userRepo;

   public function __construct(IWithdrawalRepository $repository, IEarningsRepository $earningsRepository, IUserRepository $userRepo)
   {
   ...
   public function getWithDrawalData($id = null)
   {
      if(is_admin()){
         if (empty($id)){
            $response = $this->repository->paginate(10);
         }else{
   ...
   public function getWallet($id = null)
   {
      ...
      $data = $this->earningsRepository->findOneBy(["user_id" => $id]);
      if(empty($data)){
   ...
   public function withdrawalUpdateStatus(Request $request)
   {
      ...
      if(is_withdrawal_status($status_new) && $wrq['status'] != $status_new){
         ...
         if( ($status_new == GMZ_STATUS_ACCEPT) && ($wallet['max_withdrawal'] >= $wrq['withdraw']) ){
            ...
            $updateWallet = $this->earningsRepository->updateByWhere(['user_id' => $user_id], ['balance' => $balance]);
            if ($updateWallet) {
