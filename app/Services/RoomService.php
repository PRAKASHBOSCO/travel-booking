...
class RoomService extends AbstractService implements IRoomService {
	...
	public function __construct( IRoomRepository $repository, IRoomAvailabilityRepository $roomAvaiRepo, ITermRelationRepository $termRelationRepo, ITermRepository $termRepo, IOrderRepository  $orderRepo, ICommentRepository $commentRepo) {
    ...
    public function searchRoom(Request $request){
        ...
        if(!empty($params['check_in']) && !empty($params['check_out'])){
            $unavail = $this->roomAvaiRepo->getRoomUnavailable($params);
        ...
        $number_rooms = [];
        if(!empty($params['check_in']) && !empty($params['check_out'])){
            $number_rooms = $this->roomAvaiRepo->getNumberRoomAvailability($params);
        }
    ...
    public function deletePost($request){
        ...
        $this->roomAvaiRepo->updateByWhere([
            'hotel_id' => $hotel_id,
        ], [
	...
	private function updateTerm( $post_id, $post_data ) {
		if(isset($post_data['room_facilities'])){
			...
			if(!empty($all_facilities)){
				...
				$this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'car' AND term_id IN {$facility_in_str}");
			}
	...
	public function storeTermData( $post_id ) {
		...
		if ( ! empty( $postData ) ) {
			...
			foreach ( $tax as $item ) {
				...
				$taxData = $this->termRelationRepo->where( [
					'post_id'   => $postData->id,
					'post_type' => 'post'
				] );
				...
				if ( $item == 'post_category' ) {
					...
					if ( ! empty( $res ) ) {
						$res_in   = '(' . implode( ',', $res ) . ')';
						$tagData  = $this->termRepo->whereRaw( "id IN {$res_in}" );
						$tagStore = [];
						if ( ! $tagData->isEmpty() ) {
