...
class TestimonialsService extends AbstractService implements ITestimonialsService{

    protected $repository;
    private $seoRepo;

    public function __construct(ITestimonialsRepository $repository, ISeoRepository $seoRepo)
    {
        $this->repository = $repository;
        $this->seoRepo = $seoRepo;
    }

     public function getTestimonialsPagination($number = 10, $where = []){
    ...
    public function deleteTestimonialsTemp()
    {
    ...
    public function storeNewTestimonials()
    {
        $dt=date('Y-m-d H:i:s');
...
// dd($post_data);
           ...
           $updated = $this->repository->update($post_id, $post_data);
            if($updated){
                ...
                if($finish) {
                    $response['redirect'] = dashboard_url('all-testimonials' /*. $post_id */);
                }
