...
use App\Repositories\Contracts\IPackageAvailabilityRepository;
use App\Repositories\Contracts\IPackageRepository;
...
use App\Services\Contracts\IPackageService;
...
class PackageService extends AbstractService implements IPackageService {
	...
	public function __construct( IPackageRepository $repository, IPackageAvailabilityRepository $tourAvaiRepo, ITermRelationRepository $termRelationRepo, ITermRepository $termRepo, IOrderRepository  $orderRepo, ICommentRepository $commentRepo, ISeoRepository $seoRepo) {
	...
	public function fetchTime(Request $request){
        ...
        if ($start_date && gmz_compare_hashing($postID, $postHashing)) {
            $orderData = $this->orderRepo->getOrderItems($postID, $start_date, $start_date, GMZ_SERVICE_TOUR);
    ...
    public function getRealPrice($post_id, $check_in, $check_out, $extras, $adult, $children, $infant) {
        ...
        $group_size      = (int) $post_object->group_size;
        ...
        $avails = $this->tourAvaiRepo->getDataAvailability($post_id, $check_in, $check_out);
        ...
        if ($avails) {
            ...
            $slot_select = $adult + $children;
            if($slot_select > $group_size){
                return [
                    'status' => false,
                    'message' => sprintf(__('This tour has only %s slots left'), $group_size)
                ];
            }
    ...
    public function fetchTourAvailability(Request $request) {
        ...
        if ($start_date && $end_date && gmz_compare_hashing($postID, $postHashing)) {
            $avails = $this->tourAvaiRepo->getDataAvailabilityForCalendar($postID, $start_date, $end_date);
            ...
            $orders = $this->orderRepo->getOrderItems($postID, $start_date, $end_date, GMZ_SERVICE_TOUR);
    ...
    public function getSearchResult(Request $request) {
        ...
        if(!empty($params['checkIn']) && !empty($params['checkOut'])){
            $unavail = $this->tourAvaiRepo->getListUnavailable($params);
	...
	private function updateTerm( $post_id, $post_data ) {
		if(isset($post_data['tour_type'])){
			...
			if(!empty($all_types)){
				...
				$this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'tour' AND term_id IN {$type_in_str}");
			}
		...
		if(isset($post_data['tour_include'])){
			...
			if(!empty($all_includes)){
				...
				$this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'tour' AND term_id IN {$include_in_str}");
			}
        ...
        if(isset($post_data['tour_exclude'])){
            ...
            if(!empty($all_excludes)){
                ...
                $this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'tour' AND term_id IN {$exclude_in_str}");
            }
