...
//Forum Category by vinay

if (!function_exists('get_category_permalink')) {
	function get_category_permalink($post_slug, $type)
	{
		return url("{$type}/categories/{$post_slug}");
	}
}

// Testimonials by Vinay

if (!function_exists('get_testimonials_permalink')) {
	function get_testimonials_permalink($post_slug)
	{
		return url("/testimonials/{$post_slug}");
	}
}
// Countries by vinay

if (!function_exists('get_countries_permalink')) {
	function get_countries_permalink($post_slug)
	{
		return url("/countries/{$post_slug}");
	}
}

// Inclusion by Vinay

if (!function_exists('get_inclusion_permalink')) {
	function get_inclusion_permalink($post_slug)
	{
		return url("/inclusion/{$post_slug}");
	}
}
...
if (!function_exists('get_filter_status')) {
	function get_filter_status($post_type = 'post')
	{
		...
		<div class="filter-action pl-2 mt-3">
			...
			?>
					<a href="<?php echo dashboard_url($service_name . 'all-' . $post_type . '?status=' . $kpst . '&' . $ext) ?>" class="<?php if ($status == $kpst) {
																																			echo 'text-primary font-weight-bold';
																																		} ?> mr-2">
						<?php echo $vpst ?>
					</a>
