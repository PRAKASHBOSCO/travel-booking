...
if(!function_exists('get_date_format_new')) {
	function get_date_format_new( $time = false ) {
		if ( $time ) {
			$time_type = get_option( 'time_type', '12' );
			if ( $time_type == 12 ) {
				return get_option( 'date_format', 'dS M, Y' ) . ' ' . get_option( 'time_format', 'h:i A' );
			} else {
				return get_option( 'date_format', 'dS M, Y' ) . ' ' . get_option( 'time_format', 'H:i' );
			}
		} else {
			return get_option( 'date_format', 'dS M, Y' );
		}
	}
}
...
if(!function_exists('apply_filter')) {
	function apply_filter( $action, $data ) {
		return $data;
	}
}
