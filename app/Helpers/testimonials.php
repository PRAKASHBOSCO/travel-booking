...
if (!function_exists('get_testimonials_recursive')) {
   function get_testimonials_recursive($res = [], $terms, $level = 0, $with_depth = true)
   {
      if (!$terms->isEmpty()) {
         foreach ($terms as $key => $item) {
            ...
            if (!$child->isEmpty()) {
               $new_level = $level + 1;
               $res = $res + get_testimonials_recursive($res, $child, $new_level, $with_depth);
            }
         }
      }
