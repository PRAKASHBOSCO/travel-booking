...
class SpaceService extends AbstractService implements ISpaceService {
	...
	public function __construct( ISpaceRepository $repository, ISpaceAvailabilityRepository $spaceAvaiRepo, ITermRelationRepository $termRelationRepo, ITermRepository $termRepo, IOrderRepository  $orderRepo, ICommentRepository $commentRepo, ISeoRepository $seoRepo) {
	...
	public function fetchTime(Request $request){
        ...
        if ($start_date && gmz_compare_hashing($postID, $postHashing)) {
            $orderData = $this->orderRepo->getOrderItems($postID, $start_date, $start_date, GMZ_SERVICE_SPACE);
    ...
    public function getRealPrice($post_id, $check_in, $check_out, $extras) {
        ...
        if($booking_type == 'per_day') {
            ...
            $avails = $this->spaceAvaiRepo->getDataAvailability($post_id, $check_in, $check_out);
            ...
            $orders = $this->orderRepo->getOrderItems($post_id, $check_in, $check_out, 'space');
            ...
            $orders = $this->orderRepo->getOrderItemsWithTime($post_id, $check_in, $check_out, $startTime, $endTime, 'space', $booking_type);
    ...
    public function fetchSpaceAvailability(Request $request) {
        ...
        if ($start_date && $end_date && gmz_compare_hashing($postID, $postHashing)) {
            $avails = $this->spaceAvaiRepo->getDataAvailability($postID, $start_date, $end_date);
            $space = $this->repository->find($postID);
            $price = (float)$space->base_price;
            if($bookingType == 'per_day') {
                $orders = $this->orderRepo->getOrderItems($postID, $start_date, $end_date, GMZ_SERVICE_SPACE);
            }else{
                $orders = $this->orderRepo->getOrderItemsGroupDate($postID, $start_date, $end_date, GMZ_SERVICE_SPACE);
            }
	...
	private function updateTerm( $post_id, $post_data ) {
		if(isset($post_data['space_type'])){
			...
			if(!empty($all_types)){
				...
				$this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'space' AND term_id IN {$type_in_str}");
			}
		...
		if(isset($post_data['space_amenity'])){
			...
			if(!empty($all_amenities)){
				...
				$this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'space' AND term_id IN {$amenity_in_str}");
			}
