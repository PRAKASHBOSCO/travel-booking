...
class HotelService extends AbstractService implements IHotelService {
	...
	public function __construct( IHotelRepository $repository, IRoomRepository $roomRepo, IRoomAvailabilityRepository $roomAvaiRepo, ITermRelationRepository $termRelationRepo, ITermRepository $termRepo, IOrderRepository  $orderRepo, ICommentRepository $commentRepo, ISeoRepository $seoRepo) {
    ...
    public function getRealPrice($post_id, $check_in, $check_out, $number, $equipments, $insurances ) {
        ...
        if ($number > 0) {
            ...
            $avails      = $this->roomAvaiRepo->getDataAvailability( $post_id, $check_in, $check_out );
            ...
            $orders = $this->orderRepo->getOrderItems($post_id, $check_in, $check_out, GMZ_SERVICE_HOTEL);
        ...
        if (gmz_compare_hashing($hotel_id, $hotel_hashing) && $check_in && $check_out && !empty($rooms)) {
            ...
            if(!empty($rooms)){
                foreach ($rooms as $k => $v){
                    if($v['number'] > 0){
                        $avail = $this->roomAvaiRepo->checkAvailabilityWithGuest($k, $check_in_str, $check_out_str, $v['number'], $adult, $children);
                        if($avail->isEmpty()){
                            $room_avails[] = $k;
                        }
                    }
    ...
    public function getListHotelsUnavaible($start, $end, $number_room, $number_adult, $number_child){
        ...
        $list_hotel = $this->roomAvaiRepo->getHotelUnavailable($start, $end, $number_room, $number_adult, $number_child);
    ...
    public function deletePost($request){
        ...
        $this->roomAvaiRepo->deleteByWhere([
            'hotel_id' => $post_id
        ]);

        $this->roomRepo->deleteByWhere([
            'hotel_id' => $post_id
        ]);
	...
	private function updateTerm( $post_id, $post_data ) {
		if(isset($post_data['property_type'])){
			...
			if(!empty($all_types)){
				...
				$this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'hotel' AND term_id IN {$type_in_str}");
			}
		...
		if(isset($post_data['hotel_facilities'])){
			...
			if(!empty($all_facilities)){
				...
				$this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'hotel' AND term_id IN {$facility_in_str}");
			}
        ...
        if(isset($post_data['hotel_services'])){
            ...
            if(!empty($all_services)){
                ...
                $this->termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'hotel' AND term_id IN {$service_in_str}");
            }
