...
class TermService extends AbstractService implements ITermService
{
    ...
    public function __construct(ITermRepository $repository, ITaxonomyRepository $taxRepo, IBeautyRepository $beautyRepo)
    {
	...
	public function editTerm( $request ) {
	    ...
	    if($updated){
	       ...
	       if( $tax_name == 'beauty-branch' && isset($post_data['term_location']) ){
             ...
             if ($this->beautyRepo->findOneBy(['branch' => $term_id])){
                $location_data = $post_data['term_location'];
                $sync = $this->beautyRepo->updateByWhere(
                   ['branch' => $term_id],
                   [
    ...
    public function getTaxonomyByName($tax_name){
    	return $this->taxRepo->where(['taxonomy_name' => $tax_name], true);
    }
}