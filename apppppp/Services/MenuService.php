...
class MenuService extends AbstractService implements IMenuService {
	...
	protected $menuStructureRepo;

	public function __construct( IMenuRepository $repository, IMenuStructureRepository $menuStructureRepo) {
		$this->repository       = $repository;
		$this->menuStructureRepo       = $menuStructureRepo;
	}
	...
	public function deleteMenu(Request $request) {
        ...
        if (!empty($menuObject) && is_object($menuObject)) {
            ...
            $this->menuStructureRepo->deleteByWhere([
                'menu_id' => $menu_id
            ]);
    ...
    private function _updateMenuStructure($menu_id, $menu_structure){
        $this->menuStructureRepo->resetMenuStructure($menu_id);
        if(!empty($menu_structure)){
            foreach ($menu_structure as $k => $v) {
                ...
                $this->menuStructureRepo->create($data);
            }
        }
    }

    public function getMenuItemsByMenuID($menu_id) {
        return $this->menuStructureRepo->getStructureByMenuID($menu_id);
    }
