...
class UserService extends AbstractService implements IUserService
{
   ...
   public function __construct(IUserRepository $repository, IRoleUserRepository $roleUserRepo, IEarningsRepository $earningRepo)
   {
   ...
   public function partnerRegister(Request $request)
   {
      ...
      $inserted = $this->repository->create($post_data);
      // dd($inserted);
   ...
   public function approvePartner($request)
   {
      ...
      if ($approve == 'yes') {
         $this->roleUserRepo->updateByWhere(
            ['user_id' => $user_id],
            ['role_id' => 2]
         );
         $check_exists_earning = $this->earningRepo->where(['user_id' => $user_id], true);
   ...
   public function deleteUser($request)
   {
      ...
      $this->roleUserRepo->deleteByWhere([
         'user_id' => $user_id
      ]);
      $this->earningRepo->deleteByWhere([
         'user_id' => $user_id
      ]);
   ...
   public function editUser($request)
   {
      ...
      $updated = $this->repository->update($user_id, $post_data);
   ...
   public function newUser($request)
   {
      ...
      if ($inserted) {
         ...
         if ($post_data['role'] == '2') {
            $check_exists_earning = $this->earningRepo->where(['user_id' => $inserted], true);
            if (is_null($check_exists_earning) || empty($check_exists_earning)) {
               $this->earningRepo->create([
                  'user_id' => $inserted,
                  'total' => 0,
