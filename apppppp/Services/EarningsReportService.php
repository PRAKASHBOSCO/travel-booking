...
class EarningsReportService extends AbstractService implements IEarningsReportService
{
    protected $repository;
    protected $withdrawalRepo;
    protected $orderRepo;

    public function __construct(IEarningsRepository $repository, IWithdrawalRepository $withdrawalRepo, IOrderRepository $orderRepo)
    {
    ...
    {
        return $this->orderRepo->getRecentDeals($userID, 10);
    }
    ...
    public function widgetIncomeStatistics(Request $request, $userID)
    {
        ...
        //get chart data
        $results = $this->orderRepo->getRevenue($userID, $startDate, $endDate);
    ...
    public function widgetRevenue(Request $request, $userID)
    {
        ...
        $min_date = $this->orderRepo->getMinDate($userID);
        $date_range = [
    ...
    public function widgetTotalOrders(Request $request, $userID)
    {
        ...
        $totalOrder = $this->orderRepo->totalOrders($userID);
        $statistics = $this->orderRepo->getStatisticsPerDay(7, $userID);
    ...
    public function widgetPendingTask(Request $request, $userID)
    {
        ...
        $task_order = $this->orderRepo->getPendingOrders($userID);
        $task_withdrawal = $this->withdrawalRepo->countByWhere(["status" => GMZ_STATUS_PENDING]);
