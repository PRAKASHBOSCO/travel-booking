...
class MediaService extends AbstractService implements IMediaService
{
    ...
    public function uploadMedia($request)
    {
        ...
        if (!empty($name)) {
            ...
            $folder = 'public/admin/assets/img';

            //  $saved = $file->move(storage_path($folder), $savedName);
            $saved = $file->move($folder, $savedName);
    ...
    public function getMediaFolder($storage = false)
    {
        ...
        $month = date('m');
        $date = date('d');
        return 'public/admin/assets/img';
        // if ($storage) {
